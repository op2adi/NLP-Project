...
def get_args(forced_args=None):
    ...
    parser.add_argument("--gat_heads", type=int, help="Number of attention heads in GAT layers", default=4)
    parser.add_argument("--gat_dropout", type=float, help="Dropout rate for GAT layers", default=0.1)
    ...
    parser.add_argument(
        "--bridge",
        type=str,
        choices=["dpa", "dpa+", "gcn", "gat", "multi_attention_gat", "simple"],
        )#default="xpos")
    parser.add_argument(
        "--pos_style",
        type=str,
...
def run_parser(args):
    ...
    args.td = {None: 0, "none": 0, "syn": 1, "sem": 2, "cue": 3, "scope": 4, "scope-": 5}
