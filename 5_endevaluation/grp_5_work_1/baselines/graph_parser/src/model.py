...
class BiLSTMModel(nn.Module):
    ...
    def __init__(self, vocabs, external, settings):
        ...
        if settings.ot or settings.helpers:
            ...
            elif settings.bridge == "gat":
                from gat_model import GATBridge
                self.bridge = GATBridge(
                    in_dim=settings.hidden_lstm*2, 
                    hidden_dim=int(settings.hidden_lstm / 2),
                    out_dim=settings.hidden_lstm*2,
                    heads=settings.gat_heads,
                    dropout=settings.gat_dropout
                )
            elif settings.bridge == "multi_attention_gat":
                from gat_model import MultiAttentionGATBridge
                self.bridge = MultiAttentionGATBridge(
                    in_dim=settings.hidden_lstm*2, 
                    hidden_dim=settings.hidden_lstm,
                    out_dim=settings.hidden_lstm*2,
                    num_layers=settings.gcn_layers,
                    heads=settings.gat_heads,
                    dropout=settings.gat_dropout,
                    alpha=0.2,
                    residual=True
