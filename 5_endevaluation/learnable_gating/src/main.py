...
def get_args(forced_args=None):
    ...
    parser.add_argument("--layers_lstm", type=int)
    parser.add_argument(
        "--dim_mlp",
    ...
    parser.add_argument("--dim_char_embedding", type=int)
    parser.add_argument(
        "--bridge",
        type=str,
        choices=["dpa", "dpa+", "gcn", "simple", "gat"],
        help="Which bridging module to use between sub‑models"
    )
    ...
    parser.add_argument(
        "--gat_hidden_dim",
        type=int,
        help="Hidden dimension per head in the first GATConv"
    )
    parser.add_argument(
        "--gat_dropout",
        type=float,
        help="Dropout probability in GAT layers"
    )
    parser.add_argument(
        "--pos_style",
        type=str,
    ...
    parser.add_argument(
        "--model_interpolation",
        help="model-inter = interpolation * other_loss + (1-interpolation) * primary_loss",
        type=float,
    )
    parser.add_argument(
        "--loss_interpolation",
        help="Loss = interpolation * label_loss + (1-interpolation) * edge_loss",
        type=float,
    )
    parser.add_argument(
        "--lstm_implementation",
        ...
        help=(
            "drop_connect uses stacked native layers which are then monkey patched "
            "to facilitate recurrent dropout. native is completely using built "
            "implementation, ignoring recurrent dropout."
        )
    )
    parser.add_argument(
        "--char_implementation",
        ...
        help=(
            "convolved convolves in each time step over 3 character embeddings; "
    ...
    parser.add_argument(
        "--seed", help="Sets the random seed", type=int,
    )
    parser.add_argument(
        "--force_cpu",
    ...
    parser.add_argument(
        "--early_stopping",
        help=(
            "Enables early stopping, triggered when no improvements have been "
    ...
    parser.add_argument(
        "--load", help="Load trained model", metavar="FILE",
    )
    parser.add_argument(
        "--predict_file",
        help=(
            "Skips training and instead does predictions on given file. "
    ...
    for k, v in dc.items():
        if d[k] is None:
            d[k] = v
        if d[k] is None:
            print(f"Argument {k} not set; continuing with None...")

    return args
from scorer import score
from scorer import parse_conllu_to_matrices
def predict(model, settings, to_predict, elmo, vocabs):
    ...
    with open(pred_path, "w") as fh:
        for sentence in cd.read_col_data(to_predict):
            ...
            if settings.target_style == "scope-":
                cue_matrix = sentence.make_matrix(
                    "cues", True, vocabs[settings.td["cue"]].w2i
                )
    ...
    gold_matrices = parse_conllu_to_matrices(to_predict, target_type="exp")
    pred_matrices = parse_conllu_to_matrices(pred_path, target_type="exp")
    f1, results = score(gold_matrices, pred_matrices, do_print=True)
    print(f"F1: {f1}")
...
def run_parser(args):
    ...
    if ((args.load and args.cont and not args.disable_val_eval)
            or (args.load is None and not args.disable_val_eval)):
        model = ModelInteractor.factory(args, vocabs)
