...
class BaseLSTM(nn.Module):
    def __init__(self, vocabs, external, settings):
        ...
        input_size = (
            settings.dim_embedding * (self.use_form + self.use_pos + self.use_lemma + self.use_char) +
            (external.dim * self.use_external) +
            (100 * self.use_elmo)
        )

        self.main_lstm_input = input_size
        ...
        # Gating parameters for modality fusion
        self.form_gate = nn.Parameter(torch.tensor(1.0))
        self.pos_gate = nn.Parameter(torch.tensor(1.0))
        self.lemma_gate = nn.Parameter(torch.tensor(1.0))
        self.char_gate = nn.Parameter(torch.tensor(1.0))
        self.elmo_gate = nn.Parameter(torch.tensor(1.0))

        self.ln_merge = nn.LayerNorm(self.main_lstm_input)
        self.res_proj = nn.Linear(self.main_lstm_input, 2 * settings.hidden_lstm)
    ...
    def init_embeddings(self, vocabs, external, settings):
        ...
        if not settings.disable_form:
            self.word_embedding = nn.Embedding(len(vocabs.norms) + extra, settings.dim_embedding)

        if not settings.disable_pos:
            self.pos_embedding = nn.Embedding(len(vocabs.xposs) + extra, settings.dim_embedding)

        if not settings.disable_lemma:
            self.lemma_embedding = nn.Embedding(len(vocabs.lemmas) + extra, settings.dim_embedding)
    ...
    def get_embeddings(self, indices, embedding, drop=True):
        ...
        if self.emb_dropout_type == "replace":
            unique_indices, inverse_indices = indices.unique(return_inverse=True)
            unique_indices[torch.rand(unique_indices.size()) < self.dropout_embedding] = -1
            return embedding.weight[unique_indices[inverse_indices]]
        elif self.emb_dropout_type == "zero":
            size = (embedding.weight.size(0), 1)
            mask = dropout_mask(embedding.weight.data, size, self.dropout_embedding)
            masked_embedding = embedding.weight * mask
            return masked_embedding[indices]
        else:
            raise ValueError("Unsupported embedding dropout type")
    ...
    def merge_features(self, word_indices, pos_indices, external_indices,
        ...
        if self.use_form:
            features.append(self.form_gate * self.get_embeddings(word_indices, self.word_embedding))

        if self.use_pos:
            features.append(self.pos_gate * self.get_embeddings(pos_indices, self.pos_embedding))

        if self.use_external:
            features.append(self.get_embeddings(external_indices, self.external_embedding, drop=False))

        if self.use_lemma:
            features.append(self.lemma_gate * self.get_embeddings(lemma_indices, self.lemma_embedding))

        if self.use_char:
            features.append(self.char_gate * char_features)

        if self.use_elmo and elmo_vecs is not None:
            features.append(self.elmo_gate * elmo_vecs)

        merged = torch.cat(features, 2)
        return self.ln_merge(merged)

    def forward(self, seq_lengths, chars, word_indices,
                pos_indices, external_indices, lemma_indices,
                elmo_vecs=None):

        external_indices = external_indices if self.use_external else None
        lemma_indices = lemma_indices if self.use_lemma else None

        char_features = self.char_model(chars, self.dropout_embedding) if self.use_char else None
        ...
        residual = self.res_proj(merged_features)
